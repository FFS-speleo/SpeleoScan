...
const DELETE = async (request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const resourceToChangeIndex = resources.findIndex((resource) => resource.id === resourceId);
  ...
  try {
    const data = await client.applyChange(resourcesPath, resources, `${user.email} deleted resource id: ${resourceId}`, sha);
